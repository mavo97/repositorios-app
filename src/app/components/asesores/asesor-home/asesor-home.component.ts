...
export class AsesorHomeComponent implements OnInit {
  ...
  validarAsesor() {
    ...
    this.validarcrear.validarAsesor(idF).subscribe((resp) => {
      if (resp.message === "Cuenta de correo no se encuentra registrada.") {
        ...
        this.noRegistrado = true; // console.log('Se debe registrar al asesor');
      } else {
  ...
  getAsesor() {
    this.readData
      .getAsesor(localStorage.getItem("uid"))
  ...
  getAlumnos() {
    this.readData.getAlumnos().subscribe((resp: Alumnos) => {
      this.alumnos = resp.records;
      this.loading = false;
    });
  }

  verUsuario(usuario: string) {
    this.router.navigate(["asesor/", usuario]);
  }
}
